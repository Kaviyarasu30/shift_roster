...
4. **Rules**:
   ...
   - **Minimum staff per shift** — how many people must be working Morning/Evening/Night every single day. There are two kinds of checks:
     - **Before generating**: if you simply haven't assigned enough employees to a shift to ever meet the minimum (e.g. you require 2 on Evening but only 1 employee is on Evening at all), the tool stops immediately and shows exactly which shift is short and by how much — no file is generated, since no amount of adjusting off-days could fix it.
     - **After generating**: if you do have enough people overall, but a leave happens to land on the same day as someone's weekly off, that specific day might still dip below the minimum. The Excel file still generates, but that date is listed in the **Warnings** tab so you can manually adjust (e.g. move the leave, or change who's off that day).
