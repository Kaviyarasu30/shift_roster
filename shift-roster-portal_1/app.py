...
SHIFT_COLORS = {
    MORNING: "FFE8B2",  # soft amber
    EVENING: "FFD2C2",  # soft coral
    NIGHT: "C9D6F5",    # soft indigo
    OFF: "E2E2E2",      # grey
    LEAVE: "F6C6C6",    # soft red
}
...
def build_excel(team_name, start_date, end_date, result):
    ...
    for row_offset, emp in enumerate(assignments.keys()):
        ...
        for col_offset, d in enumerate(days, start=2):
            ...
            label = {MORNING: "M", EVENING: "E", NIGHT: "N", OFF: "OFF", LEAVE: "LV"}[code]
    ...
    legend_items = [("M = Morning", MORNING), ("E = Evening", EVENING),
                     ("N = Night", NIGHT), ("OFF = Weekly Off", OFF), ("LV = Leave", LEAVE)]
